...
const SideNav = () => {
    ...
    return (
        <div className={`${navItem ? "w-48 h-full" : "w-24 mr-12 h-1/2"}   mt-16 border-2 rounded-r-lg `}>
            <div className={`${navItem ? "w-48" : "w-24 mr-12"} bg-black text-end flex justify-end rounded-tr-lg`}>
                        ...
                        {/* <!-- Sidebar content here --> */}
                        {
                            assets.map(asset =>
                                <ul className="list-disc" key={asset.asset_id}>
                                    <li className='block'><Link>{asset.asset_title}</Link></li>
                                    <hr className='mt-1' />
                                </ul>
                            )
                        }
